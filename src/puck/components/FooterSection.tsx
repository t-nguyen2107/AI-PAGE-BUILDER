...
const SOCIAL_ICONS: Record<string, string> = {
  ...
  youtube:
    "M23.5 6.19a3.02 3.02 0 00-2.12-2.14C19.5 3.5 12 3.5 12 3.5s-7.5 0-9.38.55A3.02 3.02 0 00.5 6.19 31.65 31.65 0 000 12a31.65 31.65 0 00.5 5.81 3.02 3.02 0 002.12 2.14c1.88.55 9.38.55 9.38.55s7.5 0 9.38-.55a3.02 3.02 0 002.12-2.14A31.65 31.65 0 0024 12a31.65 31.65 0 00-.5-5.81zM9.75 15.27V8.73L15.82 12l-6.07 3.27z",
  github:
...
function ScrollProgressRing({ onClick }: { onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <svg
        viewBox="0 0 20 20"
        className="w-4 h-4 fill-current text-foreground relative z-10"
      >
        <path
          fillRule="evenodd"
...
export function FooterSection(props: FooterSectionProps & ComponentMeta) {
  ...
  const colCount =
    1 +
    linkGroups.length +
    (socialLinks && socialLinks.length > 0 ? 1 : 0) +
    (newsletterIntegration ? 1 : 0);
  ...
  return (
    <>
      {backToTop && <ScrollProgressRing onClick={scrollToTop} />}

      <footer
        className={`w-full bg-primary text-foreground relative ${className ?? ""}`}
        ...
        >
          ...
          >
            ...
            <div>
              ...
              {description && (
                <p className="text-surface text-sm leading-relaxed max-w-xs">
                  {description}
                </p>
              )}
            </div>
            ...
            {linkGroups.map((group, i) => (
              <div key={i}>
                <p className="font-semibold mb-4 text-sm tracking-wide uppercase text-surface">
                  {group.title}
                </p>
                <div className="flex flex-col gap-2.5">
                  {group.links.map((link, j) => (
                    <a
                      key={j}
                      href={link.href}
                      className="text-surface text-sm inline-block hover:text-primary hover:translate-x-1 transition-all duration-200"
                    >
                      {link.label}
                    </a>
                  ))}
            ...
            {newsletterIntegration && (
              <div>
                <p className="font-semibold mb-3 text-sm tracking-wide uppercase text-surface">
                  Newsletter
                </p>
                {subscribed ? (
                  <div className="flex items-center gap-2 text-sm text-primary">
                    ...
                    <span className="text-surface">Thanks!</span>
                  </div>
                ) : (
                  <form
                  ...
                  >
                    <input
                      ...
                      className="flex-1 min-w-0 px-3 py-2 rounded-lg bg-background border border-border text-sm text-foreground placeholder:text-surface focus:outline-none focus:ring-2 focus:ring-primary/30 focus:shadow-[0_0_0_3px_hsl(var(--primary)/0.1)] transition-all duration-200"
                      required
                    />
                    <button
                      type="submit"
                      className="shrink-0 px-4 py-2 text-sm rounded-lg bg-primary text-primary-foreground font-medium hover:bg-primary/90 transition-colors duration-200"
                    >
                      Subscribe
                    </button>
                  </form>
                ...
                <div className="flex gap-2.5 flex-wrap">
                  {socialLinks.map((link, i) => (
                    <a
                      ...
                      className="w-9 h-9 rounded-lg bg-muted flex items-center justify-center text-surface hover:bg-primary hover:text-primary-foreground hover:scale-110 transition-all duration-200"
                    >
                      <svg
                        viewBox="0 0 24 24"
                        className="w-4 h-4 fill-current"
                        dangerouslySetInnerHTML={{
                          __html: getSocialIcon(link.platform),
                        }}
                      />
                    </a>
                  ))}
          ...
          {showCopyright && (
            <div className="mt-12 pt-6 border-t border-border/50 flex items-center justify-between">
              {copyright ? (
                <p className="text-surface/60 text-xs tracking-wide">
                  {copyright}
                </p>
              ) : (
                <span />
